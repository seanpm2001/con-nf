...
Roughly, any consistent and locally small specification of values of derivatives of an allowable permutation at elements of type $-1$ can be extended to some allowable permutation.

It takes some work to state the theorem precisely; we begin by setting up a framework for discussing partial specifications of structural/allowable permutations.
...
\begin{definition}
  \label{def:restrict-binary-condition}
...
\begin{proposition}
  ...
  \uses{def:conditions-from-perm,def:restrict-binary-condition}
...
\begin{proposition}
  ...
  \uses{def:condition-total-1-1,def:restrict-binary-condition}
...
\begin{definition}
  ...
  \uses{def:condition-total-1-1,def:restrict-binary-condition,def:flexible-condition}
...
Throughout this section, fix some $\alpha \in \lambda$, and assume a freedom of action context at $\alpha$.

\begin{definition}
  \label{def:strong-support}
  \uses{def:foa-context,def:constrains}
  A \emph{well-ordered support} (at level $\alpha$) is an $\alpha$-support $S$ equipped with a well-ordering $\prec_S$; it is \emph{strong} if whenever $c \in S$ and $d$ constrains $c$, then $d \in S$ and $d \prec_S c$.

  Allowable permutations act on well-ordered supports by forward image. (But note that this action may not preserve strongness, since the “constrains” relation is not $\All$-equivariant!)

  Paths act covariantly on unary conditions (compare \cref{def:restrict-binary-condition}); hence by inverse image, they act evidently on supports and well-ordered supports.  Specifically, for a support $S$ and a path $A : \beta \pathto \alpha$, $S_A = \{ (x,B) \mid (x,(B;A)) \in S \}$.
\end{definition}

\begin{lemma}
  \label{def:restrict-strong-support}
  \uses{def:strong-support}
  The restriction $S_A$ of a strong support along a path is again strong.
\end{lemma}

\begin{proof}
  Restriction \emph{reflects} (not preserves!) the “constrains” relation.  (Probably that should be an independent lemma, at least in the formalisation.)
\end{proof}

\begin{lemma}[Strenthening supports]
